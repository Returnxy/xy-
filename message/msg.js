/**
  ...
  * Follow me on Instagram @abcdefghosky_
...
module.exports = async(conn, msg, m, setting, store, welcome) => {
	try {
		let { ownerNumber,ownerNumber2 , botName, gamewaktu, limitCount } = setting
		...
		switch(command) {
			...
			case prefix+'tiktok':
			    ...
			    xfar.Tiktok(args[1]).then( data => {
			      conn.sendMessage(from, {
				   ...
				   footer: "Create by @abcdefghosky_"
			      }, { quoted: msg })
				  limitAdd(sender, limit)
			...
			case prefix+'ytmp4': case prefix+'mp4':
				...
				case prefix+'setpp2': 
					...
					await conn.query({tag : 'iq', attrs: { to: botNumber, type:'set',xlmns: 'w:profile:picture'}, content: [{tag :'picture',attrs: { type: 'image'}, content: img}]})
					await reply('sukses') } else reply('reply fotonya')
				break 
 ...
 Untuk mengambil Audio/Video dari hasil pencarian`
			...
			case prefix+'setppgrup': case prefix+'setppgc':
				...
				case prefix+'fullgc': 
					...
					if (!isBotGroupAdmins) return reply(mess.BotAdmin)
					if( isImage || isQuotedImage){
						const media = await conn.downloadAndSaveMediaMessage('image', `ppgc${from}.jpeg`)
						const {img} = await generateProfilePicture(media)
					await conn.query({tag : 'iq', attrs: { to: ppgc, type:'set',xlmns: 'w:profile:picture'}, content: [{tag :'picture',attrs: { type: 'image'}, content: img}]})
					await reply('sukses') } else reply('reply fotonya')
				break 
